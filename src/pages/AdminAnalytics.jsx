    ...
    <div className="min-h-screen p-4 md:p-8" style={{ background: "linear-gradient(135deg, #0f0c29 0%, #302b63 40%, #24243e 100%)" }}>
      <div className="max-w-4xl mx-auto space-y-3">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <button onClick={() => navigate(-1)} className="flex items-center text-gray-300 hover:text-white transition-colors text-xs mb-0.5">
              <ArrowLeft className="w-3.5 h-3.5 mr-1" /> Back
            </button>
            <h1 className="text-lg font-bold text-white">Platform Analytics</h1>
          </div>
          <div className="flex items-center gap-1.5 bg-white/10 border border-white/20 px-2.5 py-1.5 rounded-lg">
            <CalendarIcon className="w-3.5 h-3.5 text-gray-300" />
            <Select value={timeRange} onValueChange={setTimeRange}>
              <SelectTrigger className="border-0 bg-transparent text-xs font-medium text-white shadow-none focus:ring-0 min-w-[110px] h-7">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <Card className="bg-white/10 backdrop-blur-md border-white/20">
            <CardHeader className="pb-2 pt-3 px-4">
              <CardTitle className="text-white text-sm flex items-center gap-2">
                <Award className="w-3.5 h-3.5 text-yellow-400" /> Top Vendors
                <span className="text-gray-500 text-xs font-normal ml-auto">by revenue</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="px-4 pb-3">
              {analytics.topVendors.length === 0 ? (
                <p className="text-xs text-gray-400 text-center py-3">No sales data yet</p>
              ) : (
                <div className="space-y-2">
                  {analytics.topVendors.map((vendor, idx) => (
                    <div key={vendor.email} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold text-gray-500 w-3">{idx + 1}</span>
                        <VendorBadge vendor={vendor.user || { email: vendor.email }} size="sm" showRating={false} clickable={false} />
                      </div>
                      <span className="font-semibold text-xs text-white">${vendor.revenue.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <Card className="bg-white/10 backdrop-blur-md border-white/20">
            <CardHeader className="pb-2 pt-3 px-4">
              <CardTitle className="text-white text-sm flex items-center gap-2">
                <TrendingUp className="w-3.5 h-3.5 text-blue-400" /> Most Viewed Items
                <span className="text-gray-500 text-xs font-normal ml-auto">all-time</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="px-4 pb-3">
              <div className="space-y-2">
                  ...
                  <div key={item.id} className="flex items-center gap-2">
                    ...
                    <div className="w-7 h-7 rounded bg-white/10 overflow-hidden flex-shrink-0">
                      {item.images?.[0]
                        ...
                        : <ShoppingBag className="w-3.5 h-3.5 text-gray-400 m-auto mt-1.5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-white truncate">{item.title}</p>
                      <p className="text-[11px] text-gray-400">{item.views || 0} views</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <Dialog open={showRevenueDetails} onOpenChange={setShowRevenueDetails}>
          <DialogContent className="w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-base"><DollarSign className="w-4 h-4 text-green-600" /> Revenue Breakdown</DialogTitle>
              <DialogDescription className="text-xs">Take rates and fee distribution</DialogDescription>
            </DialogHeader>
            <div className="space-y-3 py-1">
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-gray-50 p-2.5 rounded-lg text-center">
                  <p className="text-[10px] text-gray-500 uppercase tracking-wide">Std Sales</p>
                  <p className="text-base font-bold mt-0.5">{analytics.revenueBreakdown.standard.count}</p>
                  <p className="text-[11px] text-gray-500">txns</p>
                </div>
                <div className="bg-green-50 p-2.5 rounded-lg text-center">
                  <p className="text-[10px] text-green-600 uppercase tracking-wide">Fees</p>
                  <p className="text-base font-bold text-green-700 mt-0.5">${analytics.revenueBreakdown.standard.fees.toLocaleString()}</p>
                  <p className="text-[11px] text-green-600">collected</p>
                </div>
                <div className="bg-blue-50 p-2.5 rounded-lg text-center">
                  <p className="text-[10px] text-blue-600 uppercase tracking-wide">Rate</p>
                  <p className="text-base font-bold text-blue-700 mt-0.5">{analytics.totalSales > 0 ? (analytics.totalRevenue / analytics.totalSales * 100).toFixed(2) : 0}%</p>
                  <p className="text-[11px] text-blue-600">blended</p>
                </div>
              </div>
              <div className="border rounded-lg overflow-hidden text-xs">
                <table className="w-full">
                  ...
                  <tbody className="divide-y">
                    <tr>
                      <td className="px-3 py-2"><div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-green-500 flex-shrink-0" /><span>Standard (12%)</span></div></td>
                      <td className="px-3 py-2 text-right text-gray-600">${analytics.revenueBreakdown.standard.volume.toLocaleString()}</td>
                      <td className="px-3 py-2 text-right text-gray-600">{analytics.revenueBreakdown.standard.count}</td>
                      <td className="px-3 py-2 text-right font-medium text-green-600">+${analytics.revenueBreakdown.standard.fees.toLocaleString()}</td>
                    </tr>
                    <tr>
                      <td className="px-3 py-2"><div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0" /><span>Exempt (0%)</span></div></td>
                      <td className="px-3 py-2 text-right text-gray-600">${analytics.revenueBreakdown.exempt.volume.toLocaleString()}</td>
                      <td className="px-3 py-2 text-right text-gray-600">{analytics.revenueBreakdown.exempt.count}</td>
                      <td className="px-3 py-2 text-right text-gray-400">$0</td>
                    </tr>
                    {analytics.revenueBreakdown.other.count > 0 && (
                      <tr>
                        <td className="px-3 py-2"><div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-orange-500 flex-shrink-0" /><span>Other</span></div></td>
                        <td className="px-3 py-2 text-right text-gray-600">${analytics.revenueBreakdown.other.volume.toLocaleString()}</td>
                        <td className="px-3 py-2 text-right text-gray-600">{analytics.revenueBreakdown.other.count}</td>
                        <td className="px-3 py-2 text-right font-medium text-green-600">+${analytics.revenueBreakdown.other.fees.toLocaleString()}</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              {analytics.revenueBreakdown.sortedExemptVendors.length > 0 && (
                <div className="bg-blue-50/50 rounded-lg p-3 border border-blue-100">
                  <h4 className="text-xs font-semibold text-blue-800 mb-2 flex items-center gap-1.5"><HelpCircle className="w-3.5 h-3.5" /> Top Exempt Drivers</h4>
                  <div className="space-y-1.5">
                    {analytics.revenueBreakdown.sortedExemptVendors.map((vendor, idx) => (
                      <div key={idx} className="flex justify-between text-xs">
                        <span className="text-blue-700 truncate mr-2">{vendor.email}</span>
                        <span className="font-medium text-blue-900 flex-shrink-0">${vendor.volume.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </div>
...
function KpiCard({ color, icon: Icon, label, value, sub, onClick, clickable }) {
  ...
  return (
    <Card
      className={`bg-white/10 backdrop-blur-md border-white/20 text-white shadow-lg transition-all ${clickable ? 'cursor-pointer hover:bg-white/15' : ''}`}
      onClick={onClick}
    >
      <CardContent className="p-3">
        <div className="flex justify-between items-start mb-1.5">
          <p className="text-[11px] font-medium text-gray-300 leading-tight">{label}</p>
          <div className="p-1 rounded" style={{ backgroundColor: `${colorMap[color]}20` }}>
            <Icon className="w-3.5 h-3.5" style={{ color: colorMap[color] }} />
          </div>
        </div>
        <h3 className="text-base font-bold text-white leading-tight">{value}</h3>
        {sub && <p className="text-[11px] text-gray-400 mt-0.5">{sub}</p>}
        {clickable && <p className="text-[10px] text-gray-500 mt-0.5">Tap for details →</p>}
      </CardContent>
    </Card>
  );
}
...
function StatBadge({ label, value, color }) {
  ...
  return (
    <div className={`rounded-lg px-3 py-2 border text-center ${colorMap[color]}`}>
